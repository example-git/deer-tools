...
def extract_form_values(action: dict, form_data: Dict[str, List[str]]) -> Dict[str, Any]:
    """
...
PROGRESS_PATTERNS = [
    ...
    re.compile(r'.*?(?:processed|completed|finished)\s+(\d+)\s+(?:of|/)\s+(\d+).*', re.IGNORECASE),
]
...
def format_progress_bar(percent: int, width: int = 30, filled_char: str = "█", empty_char: str = "░") -> str:
    """
